# Supplementary Figure S3 — leave-one-dataset-out stability of Shared-derived consensus genes
...
project_dir<-find_project_dir(get_script_dir());out_dir<-file.path(project_dir,"outputs","FigS3_outputs");dir.create(out_dir,recursive=TRUE,showWarnings=FALSE)
...
fig<-pA|pB;h<-max(6,2.2+.14*nrow(dplyr::filter(gene_stability,In_final)));ggsave(file.path(out_dir,"FigureS3.pdf"),fig,width=11,height=h,limitsize=FALSE);ggsave(file.path(out_dir,"FigureS3.tiff"),fig,width=11,height=h,dpi=600,compression="lzw",limitsize=FALSE)
ggsave(file.path(out_dir,"FigS3A_Jaccard_by_omitted_dataset.pdf"),pA,width=5.4,height=4.8)
ggsave(file.path(out_dir,"FigS3B_gene_stability.pdf"),pB,width=5.4,height=h)
write.xlsx(list(LODO_selected=lodo,Run_summary=run_summary,Gene_stability=gene_stability,Final_Fig5_genes=data.frame(Gene=final),Meta=data.frame(Rule=">=6/7 retained datasets significant in the same majority direction; no dataset is forced as anchor")),file.path(out_dir,"FigureS3_source_data.xlsx"),overwrite=TRUE)
message("Figure S3 finished: ",out_dir)
