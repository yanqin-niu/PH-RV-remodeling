# Supplementary Figure S4 — full current Figure 5 signature across all CTEPH contrasts
...
p<-ggplot(plot_df,aes(Contrast,Gene,fill=pmax(-lim,pmin(lim,log2FC))))+geom_tile(color="white",linewidth=.16)+geom_text(aes(label=mark,color=missing_value),size=1.8,show.legend=FALSE)+
 ...
 labs(title=paste0("Supplementary Figure S4. CTEPH validation of the current ",length(genes),"-gene signature"),subtitle="* adjusted P < 0.05, ** < 0.01, *** < 0.001; grey/NA = log2FC unavailable",x=NULL,y=NULL,fill="log2FC")+
...
h<-max(7,2.5+.18*length(genes));ggsave(file.path(out_dir,"FigureS4.pdf"),p,width=12,height=h,limitsize=FALSE);ggsave(file.path(out_dir,"FigureS4.tiff"),p,width=12,height=h,dpi=600,compression="lzw",limitsize=FALSE)
ggsave(file.path(out_dir,"FigS4A_full_CTEPH_heatmap.pdf"),p,width=12,height=h,limitsize=FALSE)
write.xlsx(list(Full_data=dat,Full_grid=plot_df,Gene_order=rank,Meta=data.frame(Figure5_source=f5$path)),file.path(out_dir,"FigureS4_source_data.xlsx"),overwrite=TRUE)
